(ns iso_8583.main
  "Kotodama WASM entrypoint for the Iso_8583 clean-room actor (L4) — Clojure port.
  ...
  facts (`iso_8583.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iso_8583")
(def tier "L4")
...
(def entity-specs
  [{:entity "Message"        :plural "messages"        :id-prefix "iso8583_mes"
    :fields [:mti :messageClass :messageFunction :messageOrigin :version :stan :processingCode :transmissionDateTime]
    :required [:mti :messageClass]
    :coerce {}                       :refs {}}
   {:entity "ProcessingCode"  :plural "processingcodes"  :id-prefix "iso8583_pro"
    :fields [:transactionType :accountTypeFrom :accountTypeTo]
    :required [:transactionType :accountTypeFrom]
    :coerce {}                       :refs {}}
   {:entity "DataElement"     :plural "dataelements"     :id-prefix "iso8583_dat"
    :fields [:fieldNumber :name :dataType :length :value]
    :required [:fieldNumber :name]
    :coerce {:fieldNumber :int :length :int} :refs {}}
   {:entity "ResponseCode"    :plural "responsecodes"    :id-prefix "iso8583_res"
    :fields [:code :meaning :requiresCardPickup]
    :required [:code :meaning]
    :coerce {:requiresCardPickup :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"iso_8583.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iso_8583-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iso_8583-compat/ready)
